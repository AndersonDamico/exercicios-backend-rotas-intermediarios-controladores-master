...
const adicionar = (req, res) => {
    ...
    nomeJogador = `${nomeJogador[0].toUpperCase()}${nomeJogador[0].slice(1).toLowerCase()}`
    ...
    if (indiceDoArray >= jogadores.length) {
        return res.send(`O índice informado (${indiceDoArray}) não existe no array. Novo jogador não foi adicionado.`)
    }
}
